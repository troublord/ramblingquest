...
但是有一天，刺頭工程師"小寶寶" 在修改Bug的時候打錯字了，寶哥少打了一個字母

```java
user.setRole("GUET");
```

就這樣，一個不屬於預料之中的角色"GUET" 被當成有權限的單位，在網站內叱吒風雲。
...
因為從此以後，整個專案充斥著各個工程師自定義、打錯的的角色名稱，只要蔓延到整個專案，就會牽一髮而動全身。
...
在 Java 裡，`Enum` 很適合拿來表示**固定且有限的選項**。 我們來看一下 java doc 是怎麼說 Enum的

![javadoc enum description](/images/javadoc-enum-description.webp)
